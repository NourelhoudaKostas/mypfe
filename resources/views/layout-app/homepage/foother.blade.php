<!-- Start Footer -->
<footer class="bg-dark" id="tempaltemo_footer">
    <div class="container">
        <div class="row">
            <div class="col-md-4 pt-5">
                <h2 class="h2 text-primary border-bottom pb-3 border-light logo">ESET School</h2>
                <ul class="list-unstyled text-light footer-link-list">
                    <li>
                        <i class="fab fa-whatsapp fa-fw"></i>
                        <a href="https://wa.me/2126*********" class="text-decoration-none text-light">WhatsApp Us</a>
                    </li>
                    
                    <li>
                        <i class="fa fa-phone fa-fw"></i>
                        <a class="text-decoration-none" href="tel:010-020-0340">+2126*********</a>
                    </li>
                    <li>
                        <i class="fa fa-envelope fa-fw"></i>
                        <a class="text-decoration-none" href="mailto:info@company.com">eset@gmail.com</a>
                    </li>
                </ul>
            </div>
            <div class="col-md-4 pt-5">
                <h2 class="h2 text-blue border-bottom pb-3 border-light">Further Info</h2>
                <ul class="list-unstyled text-light footer-link-list">
                    <li><a class="text-decoration-none" href="{{ route('homepage') }}">Home</a></li>
                    <li><a class="text-decoration-none" href="{{route('propos')}}">About Us</a></li>
                    {{-- <li><a class="text-decoration-none" href="{{route('Storepage')}}">FAQs</a></li> --}}
                    <li><a class="text-decoration-none" href="{{route('contact')}}">Contact</a></li>
                </ul>
            </div>

            <div class="col-md-4 pt-5">
                <h2 class="h2 text-blue border-bottom pb-3 border-light">localisation</h2>
                <iframe class="w-100" height="250" frameborder="0" style="border:0" src="https://maps.google.com/maps?q=ESET%20School&t=&z=13&ie=UTF8&iwloc=&output=embed" allowfullscreen></iframe>
            </div>

           
        </div>

        <div class="row text-light">
            <div class="col-12">
                <div class="w-100 border-top border-light"></div>
            </div>
            <div class="col-auto me-auto">
                <ul class="list-inline text-left footer-icons">
                    <li class="list-inline-item border border-light rounded-circle text-center">
                        <a class="text-light text-decoration-none" target="_blank" href="#"><i class="fab fa-facebook-f fa-lg fa-fw"></i></a>
                    </li>
                    <li class="list-inline-item border border-light rounded-circle text-center">
                        <a class="text-light text-decoration-none" target="_blank" href="#"><i class="fab fa-instagram fa-lg fa-fw"></i></a>
                    </li>
                    <li class="list-inline-item border border-light rounded-circle text-center">
                        <a class="text-light text-decoration-none" target="_blank" href="#"><i class="fab fa-twitter fa-lg fa-fw"></i></a>
                    </li>
                    <li class="list-inline-item border border-light rounded-circle text-center">
                        <a class="text-light text-decoration-none" target="_blank" href="#"><i class="fab fa-linkedin fa-lg fa-fw"></i></a>
                    </li>
                </ul>
            </div>
            <div class="col-12 text-center">
                <hr class="border-light m-0 p-0">
                <p class="mb-0">&copy; 2024 ESET School. All Rights Reserved.</p>
            </div>
        </div>
    </div>
</footer>
<!-- End Footer -->
